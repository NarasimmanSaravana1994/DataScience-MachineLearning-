...
# Create Connection
def Connector(ServerName,ServerIP,UserID,Password,Port,Database,SId):
...
def NaiveBayesTextModel(ServerName,ServerIP,UserID,Password,Database,Port,SId,Query,InDependantVariable,DependantVariable,Variable):
...
# INsert Scoring Data
def NaiveBayesScoring(ProcessedData,Data_Category):
    # sqlite connection
    ...
    CommentTrain, CommentTest, CategTrain, CategTest = train_test_split(ProcessedData, Data_Category, test_size=0.4, random_state=1)
    NaiveBayesload = joblib.load('Model.pkl')
    Vector = joblib.load('vector.pkl')
    #print()
    CommentTest_TDIDF = Vector.transform(CommentTest)
    Predicted = NaiveBayesload.predict(CommentTest_TDIDF)
    probability_0 = NaiveBayesload.predict_proba(CommentTest_TDIDF)[:, 0]
    probability_1 = NaiveBayesload.predict_proba(CommentTest_TDIDF)[:, 1]

    SqLiteCursor.execute("DROP TABLE IF EXISTS Scoring")
    SqLiteCursor.execute(
        "CREATE TABLE Scoring (Comment_Index int,Comments text, Predicted int,Probability_1 int,Probability_0 int)")
    for count in range(0, len(CommentTest)):
        SqLiteCursor.execute("INSERT INTO Scoring VALUES (?,?,?,?,?)", (
        int(CommentTest.index[count]), CommentTest.values[count],int(Predicted[count]),str(round(probability_1[count],4)),str(round(probability_0[count],4))))

    SqLiteConnect.commit()
    return True

# Scoring Method	
def NaiveScoring(ServerName,ServerIP,UserID,Password,Database,Port,SId,Query,InDependantVariable,DependantVariable,Variable):
    Connection = Connector(ServerName, ServerIP, UserID, Password, Database, Port, SId)
    Data = FetchInputData(Connection, Query)
    ...
    print(set(Data_Category))
    #ProcessedData = DataProcessing(Data_Comment)
    ScoringOutput = NaiveBayesScoring(Data_Comment,Data_Category)
